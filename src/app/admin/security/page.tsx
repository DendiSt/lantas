...
import { getSecurityGuards } from "@/app/actions/security";
import { SecurityListTable } from "@/components/admin/security/SecurityListTable";
        ...
        <header className="bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 px-6 lg:px-8 py-4 lg:py-0 lg:h-20 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">
              Kelola akun petugas keamanan gerbang
            </p>
          </div>
        </header>

        <main className="flex-1 p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto">
          <SecurityListTable guards={guards} currentUserId={session.userId} />
        </main>
